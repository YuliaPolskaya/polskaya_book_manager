...
searchForm.addEventListener('submit', (e) => {
    ...
    if (!settings.foundBookText) {
        errorBlock.innerHTML = "Введіть назву книги або автора";
    } else {
...
function createBooks(dataBody) {
    ...
    if (!data) {
        let errorMessageH4 = "<h4 class='error'>Результатів пошуку більше не виявлено</h4>";
        ...
        return (`<div  class="book">
                    ...
                    <div class="book__text">
                        <div class="book__title">
                            ...
                            <h5>${book.volumeInfo.authors ? book.volumeInfo.authors[0] : "Автор невідомий"}</h5>
                        </div>
                        <div class="book__buttons">
                            <button data-id="${id}" data-btn="click" class="btn btn__book btn--primary btn--small uppercase">Детальніше</button>
                            <!--<button class="btn btn&#45;&#45;primary btn&#45;&#45;small uppercase">Приховати</button>-->
                        </div>
                    </div>
                </div>`)
    });
...
function createModal() {
    let modal = new Modal({
        modalTitle: "Детальна інформація",
        modalText: "Desc",
        modalFooter: [,
            {
                text: "ЗАКРИТИ",
...
//подія для виклику модальних вікон
            ...
            modal.setContent(`
                <div class="modal-body">
                    ...
                    <div class="modal-text">
                        ...
                        <div class="author">
                            <h5>${data[bookID].volumeInfo.authors ? data[bookID].volumeInfo.authors[0] : "Автор невідомий"}</h5>
                        </div>
                        <div class="description">
                            <p>${data[bookID].volumeInfo.description ? data[bookID].volumeInfo.description.substr(0, 400) + "..." : "Опис не вказано."}</p>
                        </div>
                        <div class="publisher">
                            <p><strong>Видавництво: </strong>${data[bookID].volumeInfo.publisher ? data[bookID].volumeInfo.publisher : "невідомо"}</p>
                        </div>
...
//виклик модального вікна для налаштувань
function modalSettingsGenerate() {
    let settingsBtn = document.querySelector("#settings");
    settingsBtn.addEventListener("click", (e) => {
        e.preventDefault();
        $.asyncModal({
            modalTitle: "Налаштування пошуку",
            modalText: `<div>
    ...
    <form  id="search-settings">        
        <div class="settings-item">
            ...
            <label for="sort1">з релевантності</label>
            <input type="radio" id="sort2"
                   name="sort" ${settings.sortType==="newest" && "checked"} value="newest">
            <label for="sort2">Новітні</label>
        </div>
        <div class="settings-item">
            <label for="print-type">Тип друку:</label>
            <select name="printType" id="printType" form="search-settings">
                <option ${settings.printType==="all" && "selected"} value="all">Будь-який</option>
                <option ${settings.printType==="books" && "selected"} value="books">Книги</option>
                <option ${settings.printType==="magazines" && "selected"} value="magazines">Журнали</option>                
            </select>
        </div>
        <div class="settings-item">
            <label for="filter">Фільтр пошуку:</label>
            <select name="filter" id="filter" form="search-settings">
                <option ${settings.filter==="" && "selected"} value="">Без фільтрів</option>
                <option  ${settings.filter==="partial" && "selected"} value="partial" >Демо версії</option>
                <option  ${settings.filter==="full" && "selected"} value="full">Повні версії</option>
                <option  ${settings.filter==="free-ebooks" && "selected"} value="free-ebooks">Безкоштовні</option>
                <option  ${settings.filter==="paid-ebooks" && "selected"} value="paid-ebooks">Платні</option>
            </select>
        </div>
...
let setResultString = ()=>{
    ...
    if(settings.filter){
        resultString += " фільтр : "+settings.filter;
    } else if(settings.printType!=="all"){
        resultString += " тип друку : "+settings.printType;
    }
